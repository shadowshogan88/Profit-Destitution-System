...
def get_investor_rate_percent(rates: dict[int, Decimal]) -> Decimal:
    if OWN_PROFIT_LEVEL in rates:
        return rates[OWN_PROFIT_LEVEL]

    referral_total = sum(
        (rates.get(level, Decimal("0.00")) for level in range(1, MAX_REFERRAL_LEVEL + 1)),
        Decimal("0.00"),
    )
    derived = Decimal("100.00") - referral_total
    if derived < 0:
        return Decimal("0.00")
    return derived
...
def realize_profit(investment: Investment, new_profit_amount: Decimal) -> None:
    ...
    own_profit_rate = get_investor_rate_percent(rates)
    net_profit = to_2dp((new_profit_amount * own_profit_rate) / Decimal("100"))
...
def distribute_commission_upward(
    ...
    rates = get_rate_map()
    referral_levels = [level for level in sorted(rates) if level <= MAX_REFERRAL_LEVEL]
    if not referral_levels:
        return Decimal("0.00"), Decimal("0.00")
    target_max_level = max(referral_levels)
...
def distribute_profit_to_active_investors(
    ...
    for row in rows:
        ...
        own_profit_rate = get_investor_rate_percent(get_rate_map())
        won_profit = to_2dp((gross_profit * own_profit_rate) / Decimal("100"))
