...
def backfill_investment_code(apps, schema_editor):
    Investment = apps.get_model("core", "Investment")
    counters = {}

    for item in Investment.objects.all().order_by("created_at", "id"):
        ...
        item.investment_code = code_dt.strftime(f"Inv-%m/%y-{sequence:03d}")
        item.save(update_fields=["investment_code"])


class Migration(migrations.Migration):

    dependencies = [
        ("core", "0020_investmentpackage_is_popular"),
    ]
    ...
    operations = [
        ...
        migrations.RunPython(backfill_investment_code, migrations.RunPython.noop),
